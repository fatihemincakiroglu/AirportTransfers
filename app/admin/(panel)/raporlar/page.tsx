...
export default async function Page() {
  ...
  const monthly = (await sql`
    ...
    FROM bookings WHERE COALESCE(first_name, '') <> '' AND COALESCE(last_name, '') <> '' AND COALESCE(email, '') <> '' AND COALESCE(phone, '') <> '' AND created_at > now() - interval '12 months'
  ...
  const topRoutes = (await sql`
    SELECT COALESCE(dropoff, '—') AS label, COUNT(*)::int AS n,
           COALESCE(SUM(price) FILTER (WHERE status IN ('confirmed','done')), 0)::float AS revenue
    FROM bookings WHERE COALESCE(first_name, '') <> '' AND COALESCE(last_name, '') <> '' AND COALESCE(email, '') <> '' AND COALESCE(phone, '') <> '' GROUP BY 1 ORDER BY n DESC LIMIT 8`) as unknown as { label: string; n: number; revenue: number }[];

  const byVehicle = (await sql`
    SELECT COALESCE(vehicle, '—') AS label, COUNT(*)::int AS n
    FROM bookings WHERE COALESCE(first_name, '') <> '' AND COALESCE(last_name, '') <> '' AND COALESCE(email, '') <> '' AND COALESCE(phone, '') <> '' GROUP BY 1 ORDER BY n DESC LIMIT 6`) as unknown as { label: string; n: number }[];

  const byLang = (await sql`
    SELECT COALESCE(lang, '—') AS label, COUNT(*)::int AS n
    FROM bookings WHERE COALESCE(first_name, '') <> '' AND COALESCE(last_name, '') <> '' AND COALESCE(email, '') <> '' AND COALESCE(phone, '') <> '' GROUP BY 1 ORDER BY n DESC LIMIT 11`) as unknown as { label: string; n: number }[];

  const byStatus = (await sql`
    SELECT status AS label, COUNT(*)::int AS n FROM bookings WHERE COALESCE(first_name, '') <> '' AND COALESCE(last_name, '') <> '' AND COALESCE(email, '') <> '' AND COALESCE(phone, '') <> '' GROUP BY 1`) as unknown as { label: string; n: number }[];

  const byWeekday = (await sql`
    SELECT EXTRACT(dow FROM created_at)::int AS d, COUNT(*)::int AS n
    FROM bookings WHERE COALESCE(first_name, '') <> '' AND COALESCE(last_name, '') <> '' AND COALESCE(email, '') <> '' AND COALESCE(phone, '') <> '' GROUP BY 1 ORDER BY 1`) as unknown as { d: number; n: number }[];
  ...
  const [lost] = (await sql`
    ...
    FROM bookings WHERE COALESCE(first_name, '') <> '' AND COALESCE(last_name, '') <> '' AND COALESCE(email, '') <> '' AND COALESCE(phone, '') <> ''`) as unknown as { lost_revenue: number; lost_count: number; pending_revenue: number; pending_count: number }[];
  ...
  const [tot] = (await sql`
    ...
    FROM bookings WHERE COALESCE(first_name, '') <> '' AND COALESCE(last_name, '') <> '' AND COALESCE(email, '') <> '' AND COALESCE(phone, '') <> ''`) as unknown as { n: number; avg_price: number; revenue: number; won: number }[];
    ...
    <>
      ...
      <div className="mt-6 grid gap-5 lg:grid-cols-3">
        <Card className="lg:col-span-2">
          ...
          <BarChart data={monthly.map((m) => ({ label: label(m.ym), value: m.n }))} suffix="rezervasyon" />
        </Card>
        <Card>
      ...
      <div className="mt-6 grid gap-5 lg:grid-cols-2">
        ...
        <Card>
          ...
          <RankBars rows={topRoutes.slice().sort((a, b) => b.revenue - a.revenue).map((r) => ({ label: r.label, value: r.revenue }))}
                    kind="chf" accent={`linear-gradient(90deg, #059669, ${C.gold})`} />
        </Card>
        <Card>
